...
{if $locations}
    ...
    <form action="{$SITEURL}/weather" method="post">
        <select id="jojo_weather_loc" name="jojo_weather_loc" onchange="jojo_weather_update(this); return false;">
            {foreach from=$locations key=k item=v}<option value="{$k}"{if $k==$currentlocation}selected="selected"{/if}>{$v}</option>
            {/foreach}
        </select>
{elseif $location}
    <h5>{$locationdata.dnam}</h5>
{/if}
    <p>As at local time: {$locationdata.tm}</p>
    <div id="weather-current">
        <div class="weather-conditions">
            {if $currentconditions.icon}<a href='http://www.weather.com/weather/local/{$currentlocation}'><img  class='icon-image' style="vertical-align:-12px;"  alt='{if $currentconditions.t}{$currentconditions.t}{/if}' src='images/weather/31x31/{$currentconditions.icon}.png' /></a>{/if}
            {if $curr_temp && $currentconditions.tmp}Temperature: <b>{$currentconditions.tmp}</b>{$locationdata.tempunit}{if $currentconditions.tmp != $currentconditions.flik} <i>(feels like {$currentconditions.flik}{$locationdata.tempunit})</i>{/if}<br/>{/if}
            {if $wind && $currentconditions.wind}Wind: {if $currentconditions.wind.s == 'calm'}<b>Calm</b>{else}<b>{$currentconditions.wind.t}</b> @ <b>{$currentconditions.wind.s}</b>{$locationdata.speedunit}{/if}<br />{/if}
...
{foreach from=$forecasts key=k item=fc}
        {if $OPTIONS.weather_forecast_today=='yes' && $fc.hi != 'N/A' && $k==0}<h5>Forecast for Today</h5>
        {elseif $OPTIONS.weather_forecast_tomorrow=='yes' && $k>0}<h5>Forecast for {$fc.fdateday} {$fc.fdate}</h5>
        {/if}
        ...
        <div class="weather-forecast">
            ...
            {foreach from=$fc.part key=key item=fcp}{if $OPTIONS.weather_forecast_night=='yes' || $fcp.p != 'n'}<div class="weather-conditions">
                ...
                {if $wind}Wind: {if $fcp.wind.s == 'calm'}<b>Calm</b>{else}{if $fcp.wind.t}<b>{$fcp.wind.t}</b> @ {/if}<b>{$fcp.wind.s}</b>{$locationdata.speedunit}{/if}<br />{/if}
            </div>
            {/if}{/foreach}
        </div>
        {/if}
...
{/if}
    <div id="weather-credits">
        ...
        {foreach from=$weatherlinks item=wl}
            <a href="{$wl.l}">{if $wl.t}{$wl.t}{/if}</a><br />
        {/foreach}
        </p>
    </div>
